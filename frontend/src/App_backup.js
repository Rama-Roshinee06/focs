...
function App() {
  ...
  const [showTechnical, setShowTechnical] = useState({}); // Stores which donation IDs show technical details
                  ...
                  {myDonations.map(d => (
                    <div key={d._id || d.id} style={{ border: '2px solid #e5e7eb', borderRadius: '8px', padding: '16px' }}>
                      ...
                      <div style={{ marginTop: '16px', borderTop: '1px solid #f3f4f6', paddingTop: '12px' }}>
                        <button
                          onClick={() => setShowTechnical(prev => ({ ...prev, [d._id]: !prev[d._id] }))}
                          style={{
                        ...
                        >
                          {showTechnical[d._id] ? 'Hide Security Proof' : '🛡️ Show Encryption Proof'}
                        </button>
                        ...
                        {showTechnical[d._id] && (
                          <div style={{
                            ...
                            fontSize: '12px',
                            backgroundImage: 'linear-gradient(45deg, #f8fafc 25%, #f1f5f9 25%, #f1f5f9 50%, #f8fafc 50%, #f8fafc 75%, #f1f5f9 75%, #f1f5f9 100%)',
                            backgroundSize: '20px 20px'
                          }}>
                            <h4 style={{ margin: '0 0 10px 0', color: '#475569', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '5px' }}>
                              <span style={{ fontSize: '16px' }}>⚙️</span> CRYPTOGRAPHIC DATA
                            </h4>

                            <div style={{ marginBottom: '12px' }}>
                              <p style={{ color: '#64748b', margin: '0 0 4px 0', fontWeight: 'bold' }}>[TABLE CLOUD STORAGE] Ciphertext (Encrypted Phone):</p>
                              <div style={{ backgroundColor: '#1e293b', color: '#38bdf8', padding: '8px', borderRadius: '4px', wordBreak: 'break-all', borderLeft: '4px solid #38bdf8' }}>
                                {d.encryptedPhone || 'MISSING_DATA'}
                              </div>
                            </div>

                            <div style={{ marginBottom: '12px' }}>
                              <p style={{ color: '#64748b', margin: '0 0 4px 0', fontWeight: 'bold' }}>[DECRYPTED VIEW] Plaintext:</p>
                              <div style={{ backgroundColor: '#f1f5f9', color: '#0f172a', padding: '8px', borderRadius: '4px', borderLeft: '4px solid #10b981' }}>
                                {d.decryptedPhone || 'N/A'}
                              </div>
                            </div>

                            <div>
                              <p style={{ color: '#64748b', margin: '0 0 4px 0', fontWeight: 'bold' }}>[DATA INTEGRITY] Digital Signature (RSA-SHA256):</p>
                              <div style={{ backgroundColor: '#1e293b', color: '#fb7185', padding: '8px', borderRadius: '4px', wordBreak: 'break-all', borderLeft: '4px solid #fb7185' }}>
                                {d.dataHash || 'SHA256_HASH_NOT_SET'}
                              </div>
                            </div>

                            <p style={{ fontSize: '10px', color: '#94a3b8', marginTop: '10px', fontStyle: 'italic' }}>
                              * Ciphertext is stored in the database. Plaintext is only visible after decryption with the server's private key.
                            </p>
                          </div>
                        )}
                      </div>
